...
public class NotificationHelper extends ContextWrapper {
    ...
    private NotificationCompat.Builder getNotify(String text) {
        return new NotificationCompat.Builder(getApplicationContext(), CHANNEL_ID)
                ...
                .setContentTitle(getString(R.string.UpdateApp))
                .setContentText(text)
    ...
    private int getSmallIcon() {
        int icon = getResources().getIdentifier("push_small_notification", "drawable", getPackageName());
        if (icon == 0) {
    ...
    private Bitmap getLargeIcon() {
        int bigIcon = getResources().getIdentifier("push_notification", "drawable", getPackageName());
        if (bigIcon != 0) {
